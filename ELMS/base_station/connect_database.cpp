...
int main(int, char**) {
    ...
    Vehicle *v = new Vehicle(351, 10245, 1234.1234, 1234.1234, 12.0, 12, 3);
    bu.contains_id_number(v->getUnit(), index);
    bu.addToMineVehicles(v);
    if (index == -1)
    {
        newDB.addVehicle(collection_name, v->getUnit(), milliseconds(v->getTime()), v->getLongitude(), v->getLatitude(), v->getVelocity(), v->getBearing(), status = "active");
    }
    else
    {
        newDB.updateVehicle(collection_name, v->getUnit(), milliseconds(v->getTime()),
            v->getLongitude(), v->getLatitude(), v->getVelocity(), v->getBearing(), status = "active");
    }
    newDB.queryDatabase("vehicle_unit", 351);
    ...
    if (index >= 0)
    {
        newDB.updatePastData("new_bearing", 351, 32);
